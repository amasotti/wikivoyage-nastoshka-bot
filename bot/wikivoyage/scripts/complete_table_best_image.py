...
class BestImageTableCompleter(ExistingPageBot):
    ...
    def get_wikidata_id(self, name):
        ...
        wd_item = self.wb_helper.get_wikidata_entity_by_wikipedia_article_name(
            name, "", lang="it"
        )
        if wd_item:
            return wd_item
    ...
    def extract_city_name(self, cell: Tag):
        ...
        marker_template = content.filter_templates(
            matches=lambda node: node.name == "marker"
        )[
            0
        ]  # type: Template
    ...
    def process_table_row(self, cells: list[Tag]):
        ...
        marker_template = cells[1].contents.filter_templates(
            matches=lambda node: node.name == "marker"
        )[0]
